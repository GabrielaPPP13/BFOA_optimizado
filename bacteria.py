...
class bacteria():
    ...
    def tumbo(self, numSec, poblacion, numGaps, bestBacteriaIdx=None):
        """
        Inserta gaps en posiciones guiadas por la mejor bacteria para simular un tumbo inteligente.
        Inspirado en el movimiento hacia la mejor solución como en PSO.
        """
        for i in range(len(poblacion)):
            if bestBacteriaIdx is not None and i == bestBacteriaIdx:
                continue  # No modificar la mejor bacteria

            bacterTmp = list(poblacion[i])
            bestBact = list(poblacion[bestBacteriaIdx]) if bestBacteriaIdx is not None else None

            for _ in range(numGaps):
                seqnum = random.randint(0, len(bacterTmp)-1)  # selecciona secuencia

                # Si hay una mejor bacteria, usarla como guía para insertar gaps
                if bestBact:
                    gap_positions = [pos for pos, char in enumerate(bestBact[seqnum]) if char == '-']
                    if gap_positions:
                        pos = random.choice(gap_positions)
                        # Agrega ligera variación aleatoria
                        pos = max(0, min(len(bacterTmp[seqnum]), pos + random.randint(-1, 1)))
                    else:
